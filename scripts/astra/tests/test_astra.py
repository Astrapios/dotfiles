...
class TestBareCommandSessionPicker(unittest.TestCase):
    ...
    @patch.object(astra.telegram, "tg_send", return_value=1)
    @patch.object(astra.tmux, "scan_claude_sessions")
    def test_bare_status_single_message_with_picker(self, mock_scan, mock_send):
        """Bare /status is a single message carrying the inline session
        picker (no extra keyboard-restore bubble — the keyboard is kept alive
        by tg_send_receipt on routed actions instead)."""
        mock_scan.return_value = self.sessions
    ...
    def test_tg_send_receipt_rides_reply_keyboard(self):
        """tg_send_receipt attaches the persistent reply keyboard so the
        keyboard self-heals on every routed-action receipt — no separate /kb."""
        with patch.object(astra.telegram, "tg_send", return_value=7) as mock_send:
            mid = astra.telegram.tg_send_receipt("📨 Sent to w0:\n`hi`", silent=True)
        assert mid == 7
        kb = mock_send.call_args.kwargs.get("reply_markup")
        assert kb and kb.get("keyboard") and kb.get("is_persistent")
        assert mock_send.call_args.kwargs.get("silent") is True
